...
impl Client {
    async fn new() -> Result<Self, Error> {
        ...
        Ok(Self { region: region.to_string(), queue_table, queue_index, lock_table, lock_regions, queue_client, lock_clients })
    }

    async fn run_wrapper(&self, _event: LambdaEvent<Event>) -> Result<(), Error> {
        ...
        match self.run(epoch).await {
            ...
            Err(e) => {
                ...
                println!("error: {:?}", e);
                match self.record_error(execution_id.to_base62(), e, epoch).await {
    ...
    async fn run(&self, epoch: std::time::Duration) -> Result<(), Error> {
        ...
        let lock_region_index = match self.check_health(epoch).await {
            Ok((healthy, index)) => {
                if !healthy {
        ...
        // next, obtain lock
        let lock_obtained = match self.obtain_lock(epoch, lock_region_index).await {
        ...
        let lock_held = match lock_obtained {
            ...
            false => {
                ...
                match self.check_lock_held(lock_region_index).await {
                    Ok(held) => held,
                    Err(e) => {
        ...
        match lock_held {
            true => {
                ...
                return self.process_queue().await
    ...
    async fn check_health(&self, epoch: std::time::Duration) -> Result<(bool, usize), Error> {
                        ...
                        Some(items) => items.iter().all(|item| match item.get("timestamp") {
                            Some(timestamp_av) => match timestamp_av {
                                aws_sdk_dynamodb::model::AttributeValue::N(timestamp_str) => match timestamp_str.parse::<u64>() {
                                    Ok(timestamp) => timestamp >= (epoch.as_secs() - 300),
                                    Err(_) => false
                                }
                                _ => false
                            }
    ...
    async fn obtain_lock(&self, epoch: std::time::Duration, lock_region_index: usize) -> Result<bool, Error> {
        ...
        let put_result = lock_client.put_item()
            ...
            .item("ttl", dynamodb::model::AttributeValue::N((epoch.as_secs() + 90).to_string()))
            .condition_expression("attribute_not_exists(#ttl) or #ttl < :now_minus_timeout")
            .expression_attribute_names("#ttl", "ttl")
            .expression_attribute_values(":now_minus_timeout", dynamodb::model::AttributeValue::N((epoch.as_secs() - 30).to_string()))
            .send().await;
    ...
    async fn process_queue(&self) -> Result<(), Error> {
        let query_result = self.queue_client.query()
    ...
    async fn record_error(&self, execution_id: String, error: Error, epoch: std::time::Duration) -> Result<(), Error> {
        for (index, lock_client) in self.lock_clients.iter().enumerate() {
            let region = self.lock_regions.iter().nth(index).expect("invalid region index");
            let put_item_result = lock_client.put_item()
                ...
                .item("sk", dynamodb::model::AttributeValue::S(format!("error#{}", execution_id)))
                .item("region", dynamodb::model::AttributeValue::S(self.region.clone()))
                .item("timestamp", dynamodb::model::AttributeValue::N(epoch.as_secs().to_string()))
                .item("ttl", dynamodb::model::AttributeValue::N((epoch.as_secs() + 3600).to_string()))
...
async fn main() -> Result<(), Error> {
    ...
    let handler_func_closure = move |event: LambdaEvent<Event>| async move {
        client_ref.run_wrapper(event).await
    };
